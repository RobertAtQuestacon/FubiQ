...
namespace Fubi_WPF_GUI
{
	class MouseKeyboardSimulator
	{
		...
		{
			var dx = (Int32)(x*65535);
			var dy = (Int32)(y*65535);

			var input = new INPUT
			{
				Type = (UInt32) InputType.MOUSE,
				Data = {Mouse = new MOUSEINPUT {X = dx, Y = dy, Flags = (uint) (MouseFlag.ABSOLUTE | MouseFlag.MOVE)}}
			};
		...
		public static bool sendMouseButton(MouseButtonType button, bool down)
		{
			var input = new INPUT {Type = (UInt32) InputType.MOUSE};
			...
			{
				if (down)
					input.Data.Mouse.Flags = (uint)MouseFlag.MIDDLEDOWN;
				else
					input.Data.Mouse.Flags = (uint)MouseFlag.MIDDLEUP;
			}
			else if (button == MouseButtonType.X)
			{
				if (down)
					input.Data.Mouse.Flags = (uint)MouseFlag.XDOWN;
				else
					input.Data.Mouse.Flags = (uint)MouseFlag.XUP;
			}

			return sendInput(input);
		}

		public static bool sendKeyEvent(string key, bool down)
		{
			var input = new INPUT {Type = (UInt32) InputType.KEYBOARD};
			var k = (Key)Enum.Parse(typeof(Key), key);
			input.Data.Keyboard.Vk = (ushort) KeyInterop.VirtualKeyFromKey(k);
			input.Data.Keyboard.Scan = 0;
			input.Data.Keyboard.Time = 0;
			input.Data.Keyboard.ExtraInfo = IntPtr.Zero;
			input.Data.Keyboard.Flags = (uint) (down ? 0 : 2);
			return sendInput(input);
		}
