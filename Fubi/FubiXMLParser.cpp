...
bool FubiXMLParser::loadRecognizersFromXML(const std::string& fileName)
{
	...
	if (node)
	{
		...
		for(recNode = node->first_node("TemplateRecognizer"); recNode; recNode = recNode->next_sibling("TemplateRecognizer"))
		{
			...
			IGestureRecognizer* rec = parseTemplateRecognizer(recNode, globalMinConf, globalUseFilteredData, visible, name, requiredHistoryLength, getDirectoryPath(fileName));
			if (rec)
			{
				if (visible)
					core->addRecognizer(rec, name);
...
#pragma warning(pop)
	...
	{
		bool isVisible;
		rec = parseTemplateRecognizer(node, -1, false, isVisible, name, requiredHistoryLength);
	}
...
IGestureRecognizer* FubiXMLParser::parseTemplateRecognizer(xml_node<>* node, float globalMinConfidence, bool globalUseFilteredData,
	bool& isVisible, std::string& name, unsigned int& requiredHistoryLength, const std::string& baseDir /*=""*/)
{
	...
	for (xml_node<>* dataNode = node->first_node("TrainingData"); dataNode; dataNode = dataNode->next_sibling("TrainingData"))
	{
		...
		if (attr && attr->value())
		{
			...
			if (parseSkeletonFile(skeletonData, bodyMeasures, fingerCounts, startMarker, endMarker, isUser, fileName, start, end))
			{
				...
				requiredHistoryLength = maxi(requiredHistoryLength, skeletonData.size());
			}
		}
	}
	...
	if (attr)
		applyResampling = strcmp(attr->value(), "0") != 0 && removeWhiteSpacesAndToLower(attr->value()) != "false";
	...
	if (handJoints.size() > 0 && trainingData.size() > 0)
		...
		stochasticModel, numGMRStates, minConf, localTrans, useFilteredData);
		...
		useDTW, applyResampling, stochasticModel, numGMRStates, bodyMeasures, measure, minConf, localTrans, useFilteredData);
	if (rec)
		rec->m_targetSkeletonType = targetSkeleton;
	return rec;
}
