...
public:
	...
	static void showPlot(const std::vector<Fubi::Vec3f>& dataToPlot, unsigned int width, unsigned height, const std::string& windowName);
...
private:
	...
	static void applyThreshold(unsigned char* imageData, int width, int height, Fubi::ImageDepth::Depth depth, int step, float zCenter, float threshold, float replaceValue = 0);
