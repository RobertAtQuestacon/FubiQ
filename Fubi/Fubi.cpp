...
namespace Fubi
{
		...
		bool useFilteredData /*= false*/)
	{
		...
		if (core)
			...
			distanceMeasure, maxRotation, aspectInvariant, ignoreAxes, useOrientations, useDTW, applyResampling,
			stochasticModel, numGMRStates, measuringUnit,	atIndex, name, minConfidence, useLocalTransformations, useFilteredData);
		return -1;
	}

	FUBI_API unsigned int addTemplateRecognizer(const char* xmlDefinition, int atIndex /*= -1*/)
	{
		FubiCore* core = FubiCore::getInstance();
		if (core)
	...
	FUBI_API Fubi::Vec3f convertCoordinates(const Fubi::Vec3f& inputCoords, Fubi::CoordinateType::Type inputType, Fubi::CoordinateType::Type outputType)
	{
		FubiCore* core = FubiCore::getInstance();
		if (core)
	...
	FUBI_API bool startRecordingSkeletonData(const char* fileName, unsigned int targetID, bool useFilteredData /*= false*/, bool isHand /*= false*/)
	{
		FubiCore* core = FubiCore::getInstance();
		if (core)
			return core->startRecordingSkeletonData(fileName, targetID, useFilteredData, isHand);
		return false;
	}
	...
	FUBI_API bool getImage(unsigned char* outputImage, unsigned int imageIndex, Fubi::ImageNumChannels::Channel numChannels, Fubi::ImageDepth::Depth depth)
	{
		FubiCore* core = FubiCore::getInstance();
		if (core)
		{
	...
	FUBI_API const std::vector<Fubi::TemplateData>& getTemplateTrainingData(const char* recognizerName)
	{
		FubiCore* core = FubiCore::getInstance();
		if (core && recognizerName)
	...
	FUBI_API const std::vector<Fubi::TemplateData>& getTemplateTrainingData(unsigned int recognizerIndex)
	{
		FubiCore* core = FubiCore::getInstance();
		if (core)
