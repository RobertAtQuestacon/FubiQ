...
void FubiUser::calculateFilteredTransformations()
{
	...
	if (core && m_trackingData.size() > 1 && data->timeStamp > lastTrackingData()->timeStamp )
	{
...
void FubiUser::updateBodyMeasurements(double currentTimestamp)
{
	...
	if (currentTimestamp - m_lastBodyMeasurementUpdate > m_bodyMeasurementUpdateIntervall)
	{
		...
		Fubi::calculateBodyMeasurement(data->jointPositions[SkeletonJoint::WAIST],
			data->jointPositions[SkeletonJoint::NECK], m_bodyMeasurements[BodyMeasurement::TORSO_HEIGHT],filterFac);

		// Shoulder width
		Fubi::calculateBodyMeasurement(data->jointPositions[SkeletonJoint::LEFT_SHOULDER],
			data->jointPositions[SkeletonJoint::RIGHT_SHOULDER], m_bodyMeasurements[BodyMeasurement::SHOULDER_WIDTH],filterFac);

		// Hip width
		Fubi::calculateBodyMeasurement(data->jointPositions[SkeletonJoint::LEFT_HIP],
			data->jointPositions[SkeletonJoint::RIGHT_HIP], m_bodyMeasurements[BodyMeasurement::HIP_WIDTH],filterFac);

		// Arm lengths
		Fubi::calculateBodyMeasurement(data->jointPositions[shoulderToTake],
			data->jointPositions[elbowForShoulder], m_bodyMeasurements[BodyMeasurement::UPPER_ARM_LENGTH],filterFac);
		Fubi::calculateBodyMeasurement(data->jointPositions[handToTake],
			data->jointPositions[elbowForHand], m_bodyMeasurements[BodyMeasurement::LOWER_ARM_LENGTH],filterFac);
		...
		Fubi::calculateBodyMeasurement(data->jointPositions[hipToTake],
			data->jointPositions[kneeForHip], m_bodyMeasurements[BodyMeasurement::UPPER_LEG_LENGTH],filterFac);
		Fubi::calculateBodyMeasurement(data->jointPositions[footToTake],
			data->jointPositions[kneeForFoot], m_bodyMeasurements[BodyMeasurement::LOWER_LEG_LENGTH],filterFac);
...
void FubiUser::update(double currentTimestamp)
{
	...
	updateBodyMeasurements(currentTimestamp);
