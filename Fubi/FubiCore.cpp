...
void FubiCore::clearRecognitions(unsigned int targetIndex, bool isHand)
{
	...
	// User defined gestures
	unsigned int numRecs = isHand ?	m_currentHandGestureRecognitions[targetIndex].size()
	...
	for (unsigned int i = 0; i < numRecs; ++i)
	{
		...
		if (storedRecognition)
		{
			if (isHand)
                ...
                m_currentGestureRecognitions[targetIndex][i] = false;
			if (m_recognitionEndCallback)
				m_recognitionEndCallback(getUserDefinedRecognizerName(i).c_str(), targetID, isHand, RecognizerType::USERDEFINED_GESTURE);
		}
	}
	...
	// User defined combinations
	unsigned int numCombs = isHand ? m_currentHandCombinationRecognitions[targetIndex].size()
	...
	for (unsigned int i = 0; i < numCombs; ++i)
	{
		...
		if (storedRecognition)
		{
			if (isHand)
                ...
                m_currentUserCombinationRecognitions[targetIndex][i] = false;
			if (m_recognitionEndCallback)
				m_recognitionEndCallback(getUserDefinedCombinationRecognizerName(i).c_str(), targetID, isHand, RecognizerType::USERDEFINED_COMBINATION);
		}
	}
}
...
unsigned int FubiCore::addHiddenRecognizer(IGestureRecognizer* rec, const string& name, int atIndex /*= -1*/)
{
	...
	// Add recognizer
	if (atIndex < 0 || (unsigned)atIndex >= m_hiddenUserDefinedRecognizers.size())
	{
		// As a new one at the end
		atIndex = m_hiddenUserDefinedRecognizers.size();
	...
	bool useFilteredData /*= false*/)
{
	...
	if (FubiXMLParser::parseSkeletonFile(skelData, measures, fingerCounts, startMaker, endMarker, isUser, trainingDataFile, startFrame, endFrame))
	{
		...
		if (isUser)
			...
			std::vector<std::deque<Fubi::TrackingData>>(1, skelData), maxDistance, distanceMeasure, maxRotation, aspectInvariant, ignoreAxes, useOrientations, useDTW, applyResampling,
				stochasticModel, numGMRStates, measures, measuringUnit, minConfidence, useLocalTransformations, useFilteredData);
		else
			rec = createGestureRecognizer(std::vector<SkeletonHandJoint::Joint>(1, (SkeletonHandJoint::Joint)joint), std::vector<SkeletonHandJoint::Joint>(1, (SkeletonHandJoint::Joint)relJoint),
			std::vector<std::deque<Fubi::TrackingData>>(1, skelData), maxDistance, distanceMeasure, maxRotation, aspectInvariant, ignoreAxes, useOrientations, useDTW, applyResampling,
		...
		updateTrackingHistoryLength(skelData.size());
...
bool FubiCore::startRecordingSkeletonData(const char* fileName, unsigned int targetID, bool useFilteredData, bool isHand)
{
	if (isHand)
	{
		FubiHand* hand = getHand(targetID);
		if (fileName != 0x0 && hand != 0x0)
			return m_recorder->startRecording(fileName, hand, useFilteredData);
	}
	else
	{
		FubiUser* user = getUser(targetID);
		if (fileName != 0x0 && user != 0x0)
			return m_recorder->startRecording(fileName, user, useFilteredData);
	}
	return false;
}
