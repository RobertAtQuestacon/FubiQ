...
public:
	...
	TemplateRecognizer(const std::vector<Fubi::SkeletonJoint::Joint>& joints, const std::vector<Fubi::SkeletonJoint::Joint>& relJoints,
		...
		bool useDTW = true, bool applyResampling = false, bool useGMR = true, unsigned int numGMRStates = 5, const Fubi::BodyMeasurementDistance* bodyMeasures = 0x0,
	...
	TemplateRecognizer(const std::vector<Fubi::SkeletonHandJoint::Joint>& joints, const std::vector<Fubi::SkeletonHandJoint::Joint>& relJoints,
		const std::vector<std::deque<Fubi::TrackingData>>& trainingData, float maxDistance, Fubi::DistanceMeasure::Measure distanceMeasure = Fubi::DistanceMeasure::Euclidean,
		float maxRotation = 45.0f, bool aspectInvariant = false, unsigned int ignoreAxes = Fubi::CoordinateAxis::NONE, bool useOrientations = false,
		bool useDTW = true, bool applyResampling = false, bool useGMR = true, unsigned int numGMRStates = 5, float minConfidence = -1.0f, bool useLocalTransformations = false, bool useFilteredData = false);
...
protected:
	...
	float calculateDistance(const std::vector<Fubi::Vec3f>& trainingData, const Fubi::Vec3f& indicativeOrient, std::vector<Fubi::Vec3f>* testData, const std::vector<Fubi::Matrix3f>* covariances);
...
private:
	...
	// DTW options
	bool m_useDTW;
	unsigned int m_maxWarpingDistance, m_maxSampleSize;
	float m_minAabbSize;
