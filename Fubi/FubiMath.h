...
namespace Fubi
{
	...
	*/
	static bool scale(std::vector<Vec3f>& points, const Vec3f& desiredAaabbSize, float minAabbSize = 0.0f, bool keepAspect = false, bool onlyFitLargestSide = false, bool translateMinToZero = false)
	{
		...
		if (currentSize.boundingBoxVolume(true) >= minAabbSize)
		{
			if (keepAspect)
			{
				...
				else
				{
					...
					singleScale = minf(scaling.x, minf(scaling.x, scaling.z));
				}
				auto end = points.end();
				for (auto iter = points.begin(); iter != end; ++iter)
				{
	...
	/**
	* \brief Rescale a vector of points to fit a desired length using an hermitte spline interpolation
	*
	* @param inputVec the unscaled input vector
	* @param desiredSize the size to which the input vector should be scaled to
	* @param outputVec the output vector, the scaled
	*/
	static void hermitteSplineRescale(const std::vector<Vec3f>& inputVec, unsigned int desiredSize, std::vector<Vec3f>& outputVec)
	{
		unsigned int inSize = inputVec.size();
		...
		for (unsigned int i = 0; i < desiredSize-1; ++i)
		{
			...
			// Calculate the hermitte spline attributes
	...
	*/
	static int heapMedian(const std::deque<int>& vec, unsigned int windowSize)
	{
		...
		unsigned char halfSize = (windowSize / 2) + 1;
